...
def is_in_category(r , g , b , date , hour , minute , category_set):
    ...
    folder_output_prefix = ["~/Downloads/", "~/Camera/"][1]

    for k in category_set:
        print(k)
        #if (k[0] - threshold_ratio_r2g) <= ratio_r2g <= (k[0] + threshold_ratio_r2g) and (k[1] - threshold_ratio_r2b) <= ratio_r2b <= (k[1] + threshold_ratio_r2b) and (k[2]- threshold_hour) <= minute <= (k[2]+ threshold_hour) and (k[3]- threshold_minute) <= minute <= (k[3]+ threshold_minute):
        if (k[0] - threshold_ratio_r2g) <= ratio_r2g <= (k[0] + threshold_ratio_r2g) and (k[1] - threshold_ratio_r2b) <= ratio_r2b <= (k[1] + threshold_ratio_r2b) and (k[2] - threshold_ratio_g2b) <= ratio_b2r <= (k[2] + threshold_ratio_g2b) and (k[3] - threshold_ratio_g2b) <= ratio_g2r <= (k[3] + threshold_ratio_g2b) and  (k[4] - threshold_ratio_g2b) <= ratio_g2b <= (k[4] + threshold_ratio_g2b) and  (k[5] - threshold_ratio_g2b) <= ratio_g2b <= (k[5] + threshold_ratio_g2b) :
            folder_output = folder_output_prefix + f'_{k[0]}_{k[1]}_{k[2]}_{k[3]}_{k[4]}_{k[5]}/'

            if not os.path.exists(folder_output): os.makedirs(folder_output)
            return (True, folder_output)

    #category.add((ratio_r2g, ratio_r2b, ratio_g2b, int(date), int(hour), int(minute)))
    category.add((ratio_r2g, ratio_b2r, ratio_g2b, ratio_r2b, ratio_b2g, ratio_g2r))
    folder_output = folder_output_prefix + f'_{ratio_r2g}_{ratio_b2r}_{ratio_g2b}_{ratio_r2b}_{ratio_b2g}_{ratio_g2r}/'
    if not os.path.exists(folder_output): os.makedirs(folder_output)
...
def dispatch3(feature_vector):
    ...
    _is_in_category = is_in_category(feature_vector[3], feature_vector[4], feature_vector[5], date, hour, minute, category)
...
def dispatch(feature_vector):
    return dispatch3(feature_vector)

def main():
    folder_src = ['/Users/y0f00k5/Documents/b/toD/todo/screenshot20201105_ks/','/Users/y0f00k5/Pictures/screenshot/', '/Users/y0f00k5/Pictures/Monosnap/'][1] + '*'
